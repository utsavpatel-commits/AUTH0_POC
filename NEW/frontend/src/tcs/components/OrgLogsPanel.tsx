...
import { TcsSearchBar } from './TcsPageHeader';
...
export function OrgLogsPanel({ orgId }: { orgId: number }) {
  const [logs, setLogs] = useState<AuditLogEntry[]>([]);
  const [q, setQ] = useState('');

  const load = () =>
    tcsApi<AuditLogEntry[]>(`/api/tcs/orgs/${orgId}/logs${q ? `?q=${encodeURIComponent(q)}` : ''}`).then(setLogs);
  useEffect(() => { load(); }, [orgId, q]);

  return (
    <div>
      <p className="text-sm text-slate-500 mb-4">
        Audit trail for this organization — members, invitations, role changes, and security updates.
      </p>
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden ring-1 ring-slate-200/60">
        <table className="w-full text-sm">
          ...
          <tbody>
            {logs.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">No log entries for this organization yet.</td></tr>
            )}
            {logs.map((log) => (
